...
else:  # If the user chooses to use a sample dataset
...
# --- Dataset Preview ---
if df is not None:  # If a dataset is loaded
    ...
    st.write("**Missing values:**")  # Display the missing values
        ...
        if st.sidebar.button("🚀 Train Model"):  # Train model button
            ...
            try:
                ...
                if model_type == "pca":  # If the user selects PCA
                    ...
                    st.markdown("This scatter plot displays the data projected onto the first two principal components.")
                    ...
                    if len(set(cluster_labels)) > 1:  # If there is more than one cluster
                        ...
                        st.warning("Only one cluster found — silhouette score not applicable.")  # Warning if no clusters
